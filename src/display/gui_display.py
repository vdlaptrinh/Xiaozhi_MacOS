...
class GuiDisplay(BaseDisplay, QObject, metaclass=CombinedMeta):
    ...
    def _on_mode_button_click(self):
        ...
        mode_text = "Automatic dialogue" if self.auto_mode else "Manual dialogue"
    ...
    def _setup_system_tray(self):
        ...
        try:
            ...
            tray_signals = {
                ...
                "stop_music_requested": self._on_stop_music,
            }
    ...
    def _on_stop_music(self):
        ...
        try:
            ...
            app = Application.get_instance()
            if not app:
                self.logger.warning("Application 实例未找到")
                return
            ...
            if music_player:
                loop = asyncio.get_event_loop()
                if loop.is_running():
                    asyncio.ensure_future(self._stop_music_and_reset(music_player, app))
                else:
                    self.logger.warning("事件循环未运行，无法停止音乐")
            else:
                self.logger.warning("MusicPlayer 实例未找到")

        except Exception as e:
            self.logger.error(f"停止音乐失败: {e}", exc_info=True)

    async def _stop_music_and_reset(self, music_player, app):
        """
        停止音乐并重置状态（异步）.
        """
        try:
            self.logger.info("开始停止音乐...")

            # 停止音乐
            result = await music_player.stop()
            self.logger.info(f"音乐停止结果: {result}")

            # Explicitly reset audio codec
            audio_codec = getattr(app, "audio_codec", None)
            if not audio_codec and music_player:
                audio_codec = getattr(music_player, "audio_codec", None)

            if audio_codec:
                audio_codec._music_playing = False
                self.logger.info("AudioCodec: _music_playing 已设置为 False")
                
                # 重启输入流以立即启用麦克风
                try:
                    if hasattr(audio_codec, "reinitialize_stream"):
                        success = await audio_codec.reinitialize_stream(is_input=True)
                        if success:
                            self.logger.info("AudioCodec: 输入流已重启，麦克风已启用")
                        else:
                            self.logger.warning("AudioCodec: 输入流重启失败")
                except Exception as e:
                    self.logger.error(f"重启输入流失败: {e}")
            else:
                self.logger.warning("AudioCodec 未找到，无法重置 _music_playing 标志")

            # 设置设备状态为待命
            if hasattr(app, "set_device_state"):
                await app.set_device_state(DeviceState.IDLE)
                self.logger.info("设备状态已设置为 IDLE")
            else:
                self.logger.warning("Application 没有 set_device_state 方法")

            self.logger.info("音乐停止和状态重置完成")

        except Exception as e:
            self.logger.error(f"停止音乐和重置状态失败: {e}", exc_info=True)
