...
class AudioCodec:
    ...
    def _input_callback(self, indata, frames, time_info, status):
        ...
        try:
            ...
            # 步骤6: Opus编码并实时发送
            # ✅ THÊM: Tắt mic 60s khi phát nhạc (tính từ lúc bắt đầu phát)
            music_playing = getattr(self, "_music_playing", False)
            if music_playing:
                music_start_time = getattr(self, "_music_start_time", 0)
                if music_start_time > 0 and (time.time() - music_start_time) > 60:
                    # Hết 60s, bật lại mic
                    self._music_playing = False
                    music_playing = False
                    logger.info("60s已过，重新启用麦克风")
            
            if self._encoded_callback and not music_playing:
                try:
