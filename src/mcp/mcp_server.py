...
class McpServer:
    ...
    def add_common_tools(self):
        ...
        try:
            ...
            self.add_tool(
                McpTool("take_screenshot", SCREENSHOT_DESC, screenshot_properties, take_screenshot)
            )
        except Exception as e:
            logger.error(f"截图工具注册失败: {e}")
        ...
        try:
            ...
            logger.error(f"八字工具注册失败: {e}")

        # 添加台灯控制工具 - QUAN TRỌNG: Đăng ký độc lập
        try:
            from src.mcp.tools.desk_lamp import get_desk_lamp_manager
            desk_lamp_manager = get_desk_lamp_manager()
            desk_lamp_manager.init_tools(self.add_tool, PropertyList, Property, PropertyType)
            logger.info("Desk lamp tools registered successfully")
        except Exception as e:
            logger.error(f"台灯工具注册失败: {e}")

        # 添加电台工具
        try:
            from src.mcp.tools.radio import get_radio_manager
            radio_manager = get_radio_manager()
            radio_manager.init_tools(self.add_tool, PropertyList, Property, PropertyType)
        except Exception as e:
            logger.error(f"电台工具注册失败: {e}")
